...
private object XandrPigeonUtils {

  fun createConnectionError(channelName: String): XandrFlutterError {
...
data class HostAPIUserId (
...
)
 {
  ...
  override fun equals(other: Any?): Boolean {
    ...
    return XandrPigeonUtils.deepEquals(toList(), other.toList())  }
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            api.init(memberIdArg, publisherIdArg, testModeArg) { result: Result<Boolean> ->
              ...
              if (error != null) {
                reply.reply(XandrPigeonUtils.wrapError(error))
              } else {
                val data = result.getOrNull()
                reply.reply(XandrPigeonUtils.wrapResult(data))
              }
            }
          }
        } else {
          ...
          channel.setMessageHandler { _, reply ->
            api.resetController{ result: Result<Unit> ->
              ...
              if (error != null) {
                ...
                reply.reply(XandrPigeonUtils.wrapResult(null))
              }
            }
          }
        } else {
  ...
  fun onAdLoaded(viewIdArg: Long, widthArg: Long, heightArg: Long, creativeIdArg: String, adTypeArg: String, tagIdArg: String, auctionIdArg: String, cpmArg: Double, memberIdArg: Long, callback: (Result<Unit>) -> Unit)
{
    ...
    channel.send(listOf(viewIdArg, widthArg, heightArg, creativeIdArg, adTypeArg, tagIdArg, auctionIdArg, cpmArg, memberIdArg)) {
      if (it is List<*>) {
        ...
        callback(Result.failure(XandrPigeonUtils.createConnectionError(channelName)))
      } 
    }
  }
  fun onAdLoadedError(viewIdArg: Long, reasonArg: String, callback: (Result<Unit>) -> Unit)
{
  ...
  fun onNativeAdLoaded(viewIdArg: Long, titleArg: String, descriptionArg: String, imageUrlArg: String, clickUrlArg: String, customElementsArg: String, callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName = "dev.flutter.pigeon.xandr_android.XandrFlutterApi.onNativeAdLoaded$separatedMessageChannelSuffix"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(listOf(viewIdArg, titleArg, descriptionArg, imageUrlArg, clickUrlArg, customElementsArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
          callback(Result.failure(XandrFlutterError(it[0] as String, it[1] as String, it[2] as String?)))
        } else {
